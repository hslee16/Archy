...
| `archy_check` | `(path, config_path=None, contracts=False, contracts_config_path=None)` | After import changes. Direct-edge layer + SDP rules from `archy.yaml`. `contracts=True` also runs transitive import-linter contracts (replaces `archy_contracts`), nested under `contracts`; requires `archy[contracts]` (else `available=false` advisory), and is skipped when no `archy.yaml` is found. Always returns `coverage`: modules and edges the layer rules actually reach, plus per-layer sizes, so a clean `passed=true` can be told apart from a config that governs almost nothing. `min_layers_present:` in archy.yaml additionally fails the check when declared layers are absent from the tree. |
