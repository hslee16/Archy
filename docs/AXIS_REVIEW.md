...
- The best candidate if a 6th axis is ever added is **type-hint coverage** (Python-specific, clear direction, cheap to compute, actionable). **Empirics ran in 2026-05** and concluded against axis promotion: independence is the weakest archy has measured (max `|r| = 0.551`, three of five existing axes at moderate coupling), and discriminant validity is contested (django / numpy / boto3 at near-zero coverage are widely-respected codebases). The decision is to ship as a *parallel diagnostic* on `archy score`, mirroring the v0.21 call-weighted Q precedent. See [`TYPE_HINT_COVERAGE_EMPIRICS.md`](TYPE_HINT_COVERAGE_EMPIRICS.md).
...
3. ~~**If a 6th axis is ever desired, prioritize type-hint coverage** over call density.~~ Type-hint coverage was studied empirically in 2026-05 ([`TYPE_HINT_COVERAGE_EMPIRICS.md`](TYPE_HINT_COVERAGE_EMPIRICS.md)) and ruled out for axis promotion on independence and discriminant-validity grounds. The decision was to ship coverage as a parallel diagnostic, same shape as v0.21 call-weighted Q. No 6th-axis candidate is currently active.
