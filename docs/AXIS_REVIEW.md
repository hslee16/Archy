...
- The best candidate if a 6th axis is ever added is **type-hint coverage** (Python-specific, clear direction, cheap to compute, actionable).
...
3. **If a 6th axis is ever desired, prioritize type-hint coverage** over call density. Cheaper to implement, clearer direction, better discriminant validity, no shape confound.
