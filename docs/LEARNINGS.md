...
1. **Marginal real-corpus gain.** Louvain's Q advantage over greedy is large only on the regular synthetic graphs greedy handles poorly (+0.13 to +0.22); on real codebases it is marginal and sometimes negative (mean +0.008, range -0.010..+0.050). Community counts barely differ on real repos (e.g. fastapi 267/267).
2. **Louvain is not insertion-order deterministic.** Under a fixed `seed=0`, louvain is repeat-stable but insertion-order-stable on only 2/14 graphs: its partition (and any number derived from it) changes with parse order. Greedy is insertion-order invariant on all 14, measured directly here, not just cited from the [2026-06 adversarial review](https://github.com/hslee16/archy/issues/180). For the score this is disqualifying (a parse-order-dependent score breaks cross-environment reproducibility and the deliberate sentrux comparability); for the advisory DSM path it would make blocks and `diff_dsm` non-reproducible, regressing the stable-layout property the grouping exists for.
3. **Score-path switch breaks continuity.** Re-deriving the modularity axis from louvain would move it by up to +0.14 per project, breaking `.archy/history.jsonl` trend continuity and sentrux number-matching for no real-corpus quality win.
