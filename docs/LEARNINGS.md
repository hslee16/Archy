...
## v0.35.0 - MCP surface modernization (2025-2026 spec)

An audit of the `archy mcp` surface against current MCP best practice (a verified deep-research pass over the Anthropic engineering guidance and the modelcontextprotocol.io spec, tracker [#230](https://github.com/hslee16/archy/issues/230)) drove four non-breaking refinements over the existing 19 tools. The disciplining principle: improve the *surface* (annotations, response shape, error contract), do **not** add or remove tools (the 19-to-13 consolidation is breaking and was deferred to a version boundary, [#227](https://github.com/hslee16/archy/issues/227)). Four decisions were load-bearing:

- **Annotations enable client auto-approval ([#225]).** All tools now declare `readOnlyHint:true` + `destructiveHint:false` + `idempotentHint:true` + `openWorldHint:false` plus a human-friendly `title`. Under the spec's cautious defaults an un-annotated tool is assumed destructive/open-world, so a client cannot auto-approve it; declaring read-only lets a trusted client skip the confirmation prompt on every read. Wire gotcha: FastMCP's `title=` kwarg lands on `Tool.title` (top-level), not `annotations.title`.

- **Verify-first on structured output ([#228]).** The 2025-06-18 `outputSchema` + `structuredContent` contract turned out to ship *for free* via FastMCP (every tool already declared a schema and returned conforming structured content). So the ticket was not new code but making the property explicit and regression-proof: docs of the wire shape plus a conformance test validating each tool's `structuredContent` against its `outputSchema` (including the in-band error variants, which are `anyOf` members). Bare-sequence and union returns wrap under a top-level `result` key because `structuredContent` must be a JSON object.

- **Concise-by-default response shaping ([#226]).** The token-heavy `archy_dsm` / `archy_graph` gained a `response_format="summary"|"full"` enum defaulting to summary, so a routine call does not dump the whole matrix/graph into context (the DSM summary runs ~89% smaller). The full mode is capped with a `*TooLargePayload` union-refusal, reusing the existing `GraphTooLargePayload` pattern rather than inventing a `truncated: bool` flag - a typed refusal forces the agent to handle it instead of silently treating a truncated result as complete. `archy_graph`'s summary returns the same shape as `archy_graph_summary`, which deliberately sets up that tool's removal in the #227 consolidation as a mechanical change.

- **One error model, in-band for recoverable conditions ([#229]).** Error reporting had drifted into three patterns (raises, in-band `*ErrorPayload` union members, advisory fields). Standardized onto three tiers: protocol errors (FastMCP), `isError:true` for usage errors (bad argument value, malformed config, scan-too-large), and in-band results (`isError:false`) for *recoverable* conditions the agent branches on. The key choice was in-band over `isError:true` for "no usable result" cases - it keeps the structured-output conformance clean and treats "no baseline yet, snapshot first" as data, not a failure. This generalizes the exact v0.19 decision above (the `note` graceful-diagnostic, "same pattern as `archy_contracts` returning a not-available payload"): `archy_check` with no `archy.yaml` now returns an in-band `CheckErrorPayload` instead of raising, matching `archy_diff` / `archy_dsm`. A *malformed* config still raises, because a broken config is a usage error the agent cannot recover from by retrying.

## v0.36.0 - MCP tool consolidation (#227, June 2026)

The breaking final item of the #230 modernization tracker. The surface had grown to 19 tools with several overlapping clusters; tool definitions sit in the agent's context before any work, and large, similarly-named sets degrade tool-selection accuracy. v0.36 shrank it to 13 by clean removal (no aliases), folding each removed tool into a survivor via a mode/lens/param switch. Design notes:

- **Clean removal over aliases.** Keeping the old names as deprecated shims would have left `tools/list` at ~19 entries, which is exactly the cost #227 set out to cut: aliases defer the token saving rather than achieving it. The discoverability gap (an agent that knew the old name gets an unknown-tool error) is handled where agents actually look, in the *surviving* tools' descriptions ("replaces `archy_X`"), not in source comments they never read. Source comments at the deletion sites serve human maintainers only.

- **The folds reuse existing helpers; the CLI is untouched.** MCP and CLI are independent presentation layers over the same lib functions, so the consolidation is MCP-only: `archy impact`, `archy affected`, `archy hotspots`, etc. stay as separate CLI commands. `archy_impact(mode="affected")` routes to `_run_affected`; `archy_graph(focus=...)` routes to `_run_graph_focus`; the refactor lenses filter `compute_refactor_priorities` output (the lib fn is unchanged). v0.35 had already pre-shaped the graph fold by making `archy_graph(response_format="summary")` return the exact `archy_graph_summary` shape, so that removal was mechanical.

- **Lens semantics carry one documented behavior change.** `archy_what_to_refactor_next(lens="structural")` applies the `min_risk` floor that the old `archy_high_risk_modules` never had, so a very clean project can return fewer rows; callers pass `min_risk=0` to restore the old "top-N unconditionally" behavior. `lens="behavioral"` re-sorts by `hotspot_score` to match the old `archy_hotspots` ranking exactly (the fused `priority` would otherwise let a both-lens module outrank a bigger pure hotspot).

- **BC-breaking, so it landed at a version boundary.** Removing/renaming tools breaks clients pinned to the old surface, so the plugin pin lower bound moved `archy>=0.31,<1.0` -> `archy>=0.36,<1.0` (old archy versions can no longer satisfy a plugin that expects the consolidated names), with the tool-count and tool-name updates propagated across the installer's `TOOL_NAMES`, both SKILL.md mirrors, README, and the spec/agent-loop docs in lockstep.
...
## v0.37.0 - duplicate-function detection (a surfacer, not an oracle)

Shipped `archy duplicates` (CLI) + `archy_duplicates` (MCP, 14th tool): cluster functions with identical normalized AST-shape (tree-sitter, identifiers/literals folded, blake2b over the node-type stream), two-tier output (primary vs demoted same-class/boilerplate variants), with an `exact` flag for byte-identical (Type-1) clusters. Advisory only, never a score axis. The engineering was straightforward; the *research discipline* is the learning.

1. **~50% refactorability precision is the expected ceiling, not a bug.** A 94-source adversarially-verified literature review established that up to 71% of real clones are benign (Kapser & Godfrey), no production tool (SourcererCC 91%, NiCad ~99%, Deckard 93%) solves benign-vs-refactorable (they measure Type-1/2/3 *similarity*, a different question), and the boundary is genuinely semantic (a real dup and an intentional one both differ by one token; even human labelers disagree ~15%). So the tool is deliberately reframed as a same-shape *surfacer* whose semantic call is left to the reader/agent - archy's judge-not-librarian role - rather than chasing a precision number pure syntax cannot reach.

2. **Empirical honesty caught an optimistic number.** An initial 3-repo exact-tier spot-check read ~69%; a broader 12-repo, 60-cluster, domain-stratified re-validation corrected it to ~63% (95% CI [51%, 74%]) with large domain variance (ORM/SDK 80%, scientific/ML 33%). The scientific/ML crash is ~99% test-code and vendored duplication (numpy's entire exact tier is parallel test suites); source-only precision is ~74%. Small favorable samples flatter; the corpus-wide gate is what tells the truth. See [`RESEARCH_METRICS.md` §12b-§12d](research/RESEARCH_METRICS.md).

3. **Two research levers, distinctly tracked.** Precision beyond syntax needs change-history co-change (>94% in the literature, the principled demoter of intentionally-independent copies; [#131](https://github.com/hslee16/archy/issues/131)) plus cheap path-scoping of test/vendored code ([#247](https://github.com/hslee16/archy/issues/247)). Recall is capped because exact-shape-hashing is Type-1/2-only and misses Type-3 near-miss clones entirely (the plurality of real duplication); the fix is a token-overlap / MinHash primitive ([#246](https://github.com/hslee16/archy/issues/246)). The de-normalize insight (identifier normalization is a precision-reducer) is what motivated the `exact` tier and measurably helped.

4. **Additive MCP tool, no plugin-pin bump.** Growing the surface 13 -> 14 is backward-compatible, so `archy>=0.36,<1.0` stands (contrast v0.36's BC-breaking consolidation). Adding a tool still touches the known coupling points: `install.base.TOOL_NAMES`, the `test_mcp` expected-tools set, the Claude permission-seed snapshots, and both `SKILL.md` mirrors.

## v0.38.0 - v0.40.0 - resolving the two duplicate-detection levers

v0.37 point 3 named two research levers, precision beyond syntax (change-history co-change) and recall beyond Type-1/2 (a Type-3 primitive). The next three releases shipped both, and building the precision lever first produced a piece of infrastructure worth more than the duplicate feature it served.

1. **The precision lever is a general diagnostic, not a duplicate-only helper (v0.38, #131).** Change coupling shipped first as its own CLI command `archy coupling`, ranking module *pairs* that co-change in git history but share no import/call edge (Tornhill / CodeScene logical coupling, reusing the `archy hotspots` git machinery via a shared `src/archy/gitlog.py`). Because it stands alone it also became an `archy_impact(co_change=true)` overlay (the behavioral blind spot the structural blast radius misses) *before* the duplicate consumer existed. Building the shared abstraction ahead of its first internal caller is the inversion of the usual "extract once you have two call sites" rule, and it paid off: one git-history engine now feeds hotspots, coupling, the impact overlay, and duplicate demotion. Path-scoping (`#247`) landed the same release as the cheap half of the precision lever, demoting clusters wholly inside test suites or vendoring dirs. Empirics: `RESEARCH_METRICS.md` §7a.

2. **Co-change demotion lifted primary-tier precision past the syntactic ceiling (v0.39, #242).** `demote_independent` consumes the v0.38 coupling engine to demote copies that live in actively-maintained files yet never co-change (deliberately parallel implementations, the exact benign class pure syntax cannot see). This is the principled precision layer the literature put at >94%, and it moved the primary tier off the ~50% syntactic ceiling that v0.37 documented as the expected wall. It is a *default-on* demotion (`co_change=true`) because the git read is cheap and the false-positive reduction is large; projects without git degrade gracefully to the syntactic tiers. The recall-by-clone-type experiment (§12g) that motivated the next release shipped alongside, quantifying the Type-3 blind spot at ~0% before the fix.
