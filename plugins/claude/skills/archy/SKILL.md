...
| `archy_conventions` | `(path, min_family=2)` | The project's derived house style: `naming` (class-suffix families grouped by **home module**, each with `concentration`), `surfaces` (per-output-surface helper sets and cross-module mirrored names - update these together), `gates` (**finding-failure** exits only, each with its literal exit `code` and lever `flag:--strict` / `config:<attr>` / `param:<name>` / `hardcoded`, so "gate or warn?" is answered from the code), `errors` (user-error exits, counted separately), `models` (base-class, frozen, tuple-vs-list census). Read it BEFORE naming a new class or wiring a new field. Advisory; never fails. |
