...
This nests the import-linter contract results under the `contracts` field (requires `pip install archy[contracts]`; degrades to `available=false` with an advisory if the extra is absent). A failed contract means the new import violates the declared architecture; revert or restructure rather than weakening the rule. A contract that did not fail is not automatically protection: check `contracts.verified`, which is false when any contract could not have failed because its patterns matched no module.
...
| `archy_impact` | `(path, files: list[str], mode="blast", max_chains=..., depth=5, test_filter=None, co_change=False)` | `mode="blast"` (default): sizing a refactor or removal by transitive reverse-dependents (with `chains`). `mode="affected"` (replaces `archy_affected`): CI-shaped impact, returns modules pre-classified into `impacted_tests` and `impacted_modules`, depth-capped for monorepos. Use `affected` for "which tests should I run for this diff?". `co_change=True` (blast mode, opt-in) adds `co_changed`: modules that co-change with the edit in git history but have no import/call edge (the behavioral blind spot the structural blast radius misses); source-only, best-effort. |
...
| `archy_check` | `(path, config_path=None, contracts=False, contracts_config_path=None)` | After import changes. Direct-edge layer + SDP rules from `archy.yaml`. `contracts=True` also runs transitive import-linter contracts (replaces `archy_contracts`), nested under `contracts`; requires `archy[contracts]` (else `available=false` advisory), and is skipped when no `archy.yaml` is found. Always returns `coverage`: modules and edges the layer rules actually reach, plus per-layer sizes, so a clean `passed=true` can be told apart from a config that governs almost nothing. `transitive_checked` says whether this run actually evaluated the `forbid` rules transitively; when it is false and there were rules to verify, `transitive_unverified_reason` says why, naming `archy_check(contracts=True)` if it was never requested and the actual failure if it was requested and produced no verdict. So a clean pass is never silent about having seen direct edges only. `min_layers_present:` in archy.yaml additionally fails the check when declared layers are absent from the tree. `required_violations` reports unsatisfied `required:` rules (module X must transitively reach Y, counting the implicit package-`__init__` import); fix by adding the missing import, not by deleting the rule. Within `contracts`, read `verified`, not `all_kept`: a contract whose module expressions match nothing in the graph is `kept` because it could not have failed, `unverifiable` counts those, and each one carries `matched_nothing` and `unmatched_expressions` naming the patterns that matched no module (with one gap: a `.importlinter` `type = layers` contract's layer names are not covered, only its `containers`). |
...
| `archy_conventions` | `(path, min_family=2, include_tests=False)` | The project's derived house style: `naming` (class-suffix families grouped by **home module**, each with `concentration`), `surfaces` (per-output-surface helper sets and cross-module mirrored names - update these together), `gates` (**finding-failure** exits only, each with its literal exit `code` and lever `flag:--strict` / `config:<attr>` / `param:<name>` / `hardcoded`, so "gate or warn?" is answered from the code), `errors` (user-error exits, counted separately), `models` (base-class, frozen, tuple-vs-list census), `bases` (kind families by *transitive* project-defined base, with suffix agreement, so a family reached through an intermediate class is reported under the base that names it), `registries` (`NAME = Ctor(...)` families and the keywords passed), `export_gaps` and `doc_gaps` (members an `__all__` or the project's own docs leave out while listing their siblings). Read it BEFORE naming a new class or wiring a new field. Advisory; never fails. `include_tests` censuses test modules too (default false). |
| `archy_module_view` | `(path, module, include_tests=False)` | **The lookup form, for a NEGATIVE.** `archy_conventions` ranks and truncates, so a module's absence from it proves nothing; every list here is COMPLETE for the one `module` named, so absence IS the answer. Use for "does `risk` import `hotspots`", "does anything still reach `layers`", "is this module dead". Returns `imports_internal` (relative imports resolved), `imported_by` (test importers counted even when tests are otherwise set aside), `classes`, `functions`, `exports` (null when there is no `__all__` and no explicit re-export, which differs from an empty one), `suffix_families`, `gates`, and `status` (censused, or set aside and why - so an empty result is never read as "nothing to report"). Raises on an unknown module name. Advisory; mutates nothing. |
