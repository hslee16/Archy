...
| `archy_conventions` | `(path, min_family=2)` | The project's derived house style: `naming` (class-suffix families + home module + `concentration`), `surfaces` (per-output-surface helper sets and cross-module mirrored names - update these together), `gates` (non-zero exit sites, each tagged `flag:--strict` / `config:<attr>` / `param:<name>` / `hardcoded`, so "gate or warn?" is answered from the code), `models` (base-class, frozen, tuple-vs-list census). Read it BEFORE naming a new class or wiring a new field. Advisory; never fails. |
